module.exports = function (app) {
  ...
  plugin.start = (options, restartPlugin) => {
    ...
    // Initial declarations
    {
      ...
      const lastCalculation = initSteps();
    ...
    // calculate deltas from input
    {
      function processDeltas(timestamp) {
        ...
        if (options.calculateGroundWind) sendGroundWind(daddStep(calc, "dampen ground wind", ampenedGroundWind.update(addStep(calc, "calculate ground wind", substract(wind, groundSpeed), timestamp))));
        addBoat(calc, boat);
    ...
    // Store calculations for examination and display
    {
      ...
      function addBoat(calculations, boat, ground) {
        ...
        calculations.boatSpeed = { speed: toKnots(boat.speed), angle: toAngle(boat.angle) };
        calculations.groundSpeed = { speed: toKnots(ground.speed), angle: toAngle(ground.angle) };
      }
    }
  }
